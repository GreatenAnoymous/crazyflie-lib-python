...
def flip360(cf,t,dt):
    ...
    for i in range(0,n):
        ...
        rate[1]=omega_dx(ti)*np.pi/180
        
        pitch=pitch+rate[1]*dt
        rpy[1]=pitch
  
        if ti<=0.52:
            if ti<=0.26:
                cf.commander.send_full_state_setpoint(r,v,a,rpy,rate)
            else:
                cf.commander.send_full_state_setpoint(r,v,a,rpy,rate)
        #cf.send_full_state_setpoint(r,v,a,rpy,rate)
            time.sleep(dt) 
        else:   #re-stablization!
            cf.commander.send_position_setpoint(0,0,hovering_z,0)
            time.sleep(dt) 
        
def flip360X(cf,t,dt):
    n=int(t/dt)
    ...
    for i in range(0,n):
        ...
        ratePitch=omega_dx(ti)
        
        pitch=pitch+ratePitch*dt
       
        print(pitch)
        if ti<=0.52:
            if ti<=0.26:
                cf.commander.send_attitude_thrust_setpoint(0,pitch,0,0,ratePitch,0,32000)
            else:
                cf.commander.send_attitude_thrust_setpoint(0,pitch,0,0,ratePitch,0,32000)
        #cf.send_full_state_setpoint(r,v,a,rpy,rate)
            time.sleep(dt) 
...
def test_hover(cf,dt):
    hovering_z=0.3
    ts=3.0
    n=int(ts/dt)
    for i in range(0,n):
        cf.commander.send_position_setpoint(0,0,hovering_z*i/n,0)
        time.sleep(dt)
...
def test_hover3(cf,dt):
    ...
    for i in range(0,n):
        r[2]=hovering_z-hovering_z*i/n
        cf.commander.send_full_state_setpoint(r,v,a,rpy,rate)
        time.sleep(dt)
        
    np.savetxt("vel_data.txt",lg.vel_data,fmt="%.1f")
    np.savetxt("data.txt",lg.data,fmt='%.3f')
    #np.savetxt("motor_data.txt",lg.motor_data,fmt='%.1f')
    np.savetxt("cmd_data.txt",lg.cmd_data,fmt="%.1f")
    np.savetxt("rpy_data.txt",lg.rpy_data,fmt="%.1f")
...
def test_angular(cf,dt):
    ...
    n=int(tc/dt)
    for i in range(0,n):
        cf.commander.send_hover_setpoint(0,0,0,hovering_z+1.0*i/n)
        time.sleep(dt)
